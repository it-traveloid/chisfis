...
const FlightDateRangeInput: FC<FlightDateRangeInputProps> = ({
  ...
  const renderInputpickUpDate = (enableSubmit = true) => {
  ...
  return (
    ...
    >
        ...
        {/* {dropOffLocationType === "oneWay" ? (
          <SingleDatePicker
            ...
            onFocusChange={({ focused }) => {
              handleDateFocusChange(focused ? "startDate" : "endDate");
            }}
            noBorder
      ...
      <div className={`flex-1 ${wrapFieldClassName}`}>
        {renderInputpickUpDate(dropOffLocationType === "oneWay")}
        {dropOffLocationType === "roundTrip" && renderInputdropOffDate()}
      </div>
    </div>
  );
};
